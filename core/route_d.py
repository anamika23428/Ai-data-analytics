...
def _generate_sql(prompt: str, column_dict: str, tables: list[str], conn) -> dict:
    ...
    try:
        ...
        return {"success": False, "error": technical}
...
def run(
    ...
    try:
        ...
        return RouteDResult(
            success=False, route=route_label,
            error=f"Query execution failed: {technical}",
            sql=sql,
        )
    ...
    # ── Step 5: AI observation (via shared insight_engine) ───────────────────
    try:
        from core.ddl_utils import generate_multi_table_ddl
        ddl = generate_multi_table_ddl(conn, tables) if ddl_schema is None else ddl_schema
        observation = generate_natural_language_insight(prompt, ddl, sql, df)
    except Exception as exc:
        technical = str(exc)
        logger.error("Route D observation failed: %s", technical)
        return RouteDResult(
            success=False, route=route_label,
            error=f"The query ran successfully but the AI observation step failed: {technical}",
            dataframe=df,
            sql=sql,
        )
